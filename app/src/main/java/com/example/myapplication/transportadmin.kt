...
import com.example.myapplication.Adapters.transportadminAdapter
 ...
 * Use the [transportadmin.newInstance] factory method to
 * create an instance of this fragment.
 */
class transportadmin : Fragment() {
    // TODO: Rename and change types of parameters
    private var param1: String? = null
    private var param2: String? = null
    private lateinit var searchView: SearchView
    private lateinit var adapter: transportadminAdapter
    private lateinit var mList: List<transport>
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_transportadmin, container, false)

        val recyclerview = view.findViewById<RecyclerView>(R.id.recyclerviewtransnportss)

        // this creates a vertical layout Manager
        recyclerview.layoutManager = LinearLayoutManager(requireContext())
        searchView = view.findViewById(R.id.searchView)
        // initialize the adapter with an empty list
        adapter = transportadminAdapter(emptyList())

        recyclerview.adapter = adapter

        // fetch data from the API
        ApiHelper().fetchtransports { transports ->
            requireActivity().runOnUiThread {
                // initialize mList with the fetched data
                mList = transports
                // update the adapter with the fetched data
                adapter = transportadminAdapter(mList)
                recyclerview.adapter = adapter

                adapter.setOnClickListener { userId ->
                    Toast.makeText(requireContext(), "Selected transport id: $userId", Toast.LENGTH_SHORT).show()
